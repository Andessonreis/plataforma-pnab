...
export function DeleteSlideButton({ slideId, slideTitle }: DeleteSlideButtonProps) {
  ...
  async function handleDelete() {
    const confirmed = window.confirm(
      `Tem certeza que deseja excluir o slide "${slideTitle}"? Esta ação não pode ser desfeita.`,
    )
    if (!confirmed) return
    ...
    try {
      ...
      if (!res.ok) {
        ...
        alert(data.message || 'Erro ao excluir slide.')
        return
      }
  ...
  return (
    <Button
      ...
      aria-label={`Excluir slide ${slideTitle}`}
    >
