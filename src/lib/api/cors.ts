...
export function corsHeaders(req: NextRequest): Record<string, string> {
  ...
  // Rejeita explicitamente origin: null (iframes sandboxed, redirects cross-origin)
  // Rejeita se API_CORS_ORIGINS não configurado (sem whitelist = sem acesso)
  const isAllowed = origin !== 'null' && origin !== '' && allowed.length > 0 && allowed.includes(origin)
